...
function KebunVirtualPage() {
  ...
  useEffect(() => {
    ...
    // Fetch kebun detail
    fetch(`http://localhost:8000/api/kebun?user_id=me`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(res => res.json())
      .then(data => {
        setKebunList(Array.isArray(data) ? data : []);
        // Cari kebun sesuai id
      ...
      <div className="flex">
        ...
        <div className="flex-1 ml-[96px] mt-15"> {/* Sesuaikan offset sesuai lebar sidebar (w-24 = 96px) */}
          <div className="min-h-screen bg-white p-6 flex flex-col gap-4">
            ...
            <div className="mb-4">
              {loading ? (
                <div className="text-[#3B5D2A]">Memuat data kebun...</div>
              ) : kebun ? (
                <div className="bg-[#EAF3E2] rounded-lg p-4 flex flex-col md:flex-row gap-6 shadow border border-[#D6E5C2]">
                  <div className="flex-1">
                    <div className="text-lg font-bold text-[#3B5D2A] mb-2">Informasi Kebun</div>
                    <div className="text-[#3B5D2A]">Nama Kebun: <span className="font-semibold">{kebun.nama_kebun}</span></div>
                    <div className="text-[#3B5D2A]">Ukuran: <span className="font-semibold">{kebun.panjang} x {kebun.lebar} m</span></div>
                    <div className="text-[#3B5D2A]">ID Kebun: <span className="font-semibold">{kebun.id}</span></div>
                  </div>
                </div>
              ) : (
                <div className="text-red-500">Kebun tidak ditemukan atau tidak ada akses.</div>
              )}
            </div>
